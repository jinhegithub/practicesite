...
if (!empty($errors)) { //if there are any errors
	...
	if (($res = register($uname, $passwd)) === false) { //if database error
      ...
      echo "<script>alert('자료등록성공!')</script>";
			if($gotohome+=1) header('Location: index.php'); //after sucessful register goto the login page
			die();
		}
		else {
      echo "<script>alert('자료등록성공!')</script>";
      if($gotohome+=1) header('Location: index.php'); //you can replace this redirect with one to the chat page of your site if you want
			die();
		}
	}
}

echo "<script>alert('자료등록실패!')</script>";

//else redirect to the registration form of the site
header('Location: index.php'); //you can replace this redirect with one to the chat page of your site
die();
